    ...
    match piece.position with
      ...
      | Some p ->
        if piece.nameOfType = "king" then
          ...
          let mutable vacant1 = List.collect fst vacantPieceLists
          // Extract and merge lists of first obstruction pieces and filter out own pieces
          let mutable opponent1 = List.choose snd vacantPieceLists
          for k in 0..7 do
                for j in 0..7 do
                    if _board.[k, j].IsSome && _board.[k, j].Value.position <> piece.position  then
                        let availableMoves1 =
                            
                               match _board.[k, j].Value.position with
                                  None ->
                                      ([], [])
                                  | Some p ->
                                     let convertNWrap = 
